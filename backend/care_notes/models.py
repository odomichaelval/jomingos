...
class VoiceCareNoteDraft(models.Model):
    """Temporary draft created from voice recording, pending staff review."""
    patient = models.ForeignKey(Patient, on_delete=models.CASCADE)
    recorded_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True)
    audio_file = models.FileField(upload_to='voice_care_notes/', blank=True, null=True)
    transcript = models.TextField(blank=True)          # raw Whisper output

    # AI-structured draft value (mirrors CareNote.note_text)
    structured_note = models.TextField(blank=True)     # cleaned-up, professional version
    ...
    def __str__(self):
        return f'Voice care note draft for {self.patient} at {self.created_at}'

# AI Summary Evaluation & Explainability

class CareNoteSummaryEvaluation(models.Model):
    """
    Stores each AI-generated period summary along with its evaluation
    metrics (ROUGE/BLEU/BERTScore against the source notes) and its
    evidence-linking data (which original notes support which parts
    of the summary), for dissertation reporting and staff transparency.
    """
    patient = models.ForeignKey(Patient, on_delete=models.CASCADE)
    ...
    # Evaluation metrics — faithfulness/overlap of summary vs. concatenated source notes
    ...
    bertscore_precision = models.FloatField(null=True, blank=True)
    bertscore_recall = models.FloatField(null=True, blank=True)
    bertscore_f1 = models.FloatField(null=True, blank=True)

    # Explainability — list of {summary_sentence, matched_note_id, matched_note_date, similarity}
    ...
    def __str__(self):
        return f'Summary evaluation for {self.patient} ({self.period_start} to {self.period_end})'
